...
class LFWDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_path, transforms=None, download=False, split_name: str = 'train'):
        super().__init__()
        
        if download:
            self._download_resources(dataset_path)
    ...
    def __getitem__(self, idx):
        value = self._dataset[idx]
        if self._transforms:
            value = (self._transforms(value[0]), self._transforms(value[1]))
    ...
    def _load_dataset(self, dataset_path: Path):
        input_groundtruth_pair_list = self._get_list_of_input_groundtruth_pairs(dataset_path)
        input_groundtruth_data = self._get_list_of_input_and_groundtruth_data_pairs(input_groundtruth_pair_list)

        return input_groundtruth_data


    def _get_list_of_input_and_groundtruth_data_pairs(self, input_groundtruth_pair_list):
        input_grountruth_pair_list = []
        for sample_path, segmentation_path in input_groundtruth_pair_list:
            sample = cv2.imread(sample_path)
            sample = cv2.cvtColor(sample, cv2.COLOR_BGR2RGB)
            segmentation = cv2.imread(segmentation_path)
            segmentation = cv2.cvtColor(segmentation, cv2.COLOR_BGR2RGB)
        
            input_grountruth_pair_list.append((sample, segmentation))
        
        return input_grountruth_pair_list


    def _get_list_of_input_groundtruth_pairs(self, dataset_path: Path) -> list:
        sample_path = dataset_path / 'lfw_funneled'
        segmentation_path = dataset_path / 'parts_lfw_funneled_gt_images'
        image_path_list = self._get_list_of_image_paths(sample_path)

        input_groundtruth_pair_list = []
        for image_path in image_path_list:
            segmentation_path = self._get_segmentation_path(image_path)
            if os.path.exists(segmentation_path):
                input_groundtruth_pair_list.append((image_path, segmentation_path))
        
        return input_groundtruth_pair_list


    def _get_list_of_image_paths(self, directory: Path) -> list:
        image_path_list = []
    ...
    def _get_segmentation_path(self, image_path):
        return 'D:\ML\cv_dl\lfw_dataset\parts_lfw_funneled_gt_images\\' + os.path.basename(image_path).split('.')[0] + '.ppm'


    def _download_resources(self, base_folder):
